...
output "active_rsa_public_key" {
    value       = local.latest_rsa_public_key_number == 1 ? jsondecode(aws_lambda_invocation.lambda_function.function_name)["rsa_public_key_1"] : jsondecode(aws_lambda_invocation.lambda_function.function_name)["rsa_public_key_2"]
